...
local function sgd(opfunc, x, config, state)
   -- (0) get/update state
   -- ADDED BY CADENE
   local pbfc = config.paramsBeforeFullyConnected
   local fcf = config.learningRateConvFactor
   --
   ...
   if lrs then
      ...
      if pbfc then
         x:narrow(1, 1, pbfc):add(-clr/fcf, dfdx:narrow(1, 1, pbfc))
         x:narrow(1, pbfc+1, x:size(1)-pbfc):add(-clr, dfdx:narrow(1, pbfc+1, x:size(1)-pbfc))
      else
        x:add(-clr, dfdx)
      end
   end
